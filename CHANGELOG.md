...
- Two delivery channels through one `provider` option: `direct`
  (`https://data.skylinkapi.com/v3.1`, `x-api-key`, `$SKYLINK_API_KEY`) and `rapidapi`
  (`https://skylink-api.p.rapidapi.com`, `X-RapidAPI-Key`/`X-RapidAPI-Host`,
  `$RAPIDAPI_KEY`). A custom `base_url` is used verbatim and makes the key optional, so
  staging instances running `DISABLE_AUTH=true` work out of the box.
...
- `client.last_rate_limit`: `RateLimitInfo(limit, remaining, reset)` parsed from the
  `X-RateLimit-Requests-*` headers of the last successful response.
...
- `skylink-api` on PyPI (import `skylink_api`), hatchling build, src layout, Python 3.10+,
  runtime dependencies `httpx>=0.25,<1` and `pydantic>=2.5,<3`. MIT licensed.
...
- 662 unit tests, network free (`respx` mocks, injected backoff sleeps), plus an
  environment-gated integration suite (`SKYLINK_TEST_BASE_URL`).
- Six runnable scripts in `examples/` and a full method index in the README.
